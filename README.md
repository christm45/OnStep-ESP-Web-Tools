...
FILES TU USE FOR THE FIRMWARE BUILD ARE THE.BAT ONES...
1 RUN bin_maker.bat
...
### Step 3: Sync manifests and manifest list

If you add or replace `.bin` files, run:

```powershell
.\sync_firmware.ps1
```

This generates per‑version manifest JSON files and updates `manifest_list.js` used by `index.html`.

### Step 4: Use the web interface

1. Double-click `start_webtool.ps1` to launch a local server and open `http://localhost:8000/index.html`.
   - It uses Python if present, else `npx serve`, else a built-in PowerShell server. The window stays open until you press Enter.
...
- Run `.\sync_firmware.ps1` to auto-create the manifest and regenerate the list used by the UI.
- Launch `.\start_webtool.ps1` and flash from the interface.
...
├── start_webtool.ps1      # Local server launcher
├── sync_firmware.ps1      # Auto-create manifests + manifest_list.js
